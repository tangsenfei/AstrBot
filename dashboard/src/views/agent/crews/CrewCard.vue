<template>
  <v-card class="crew-card" hover>
    <v-card-text class="pb-2">
      <div class="d-flex align-start justify-space-between mb-2">
        <div class="flex-grow-1">
          ...
          <v-chip :color="processColor" size="x-small" class="mt-1">
            {{ processLabel }}
          </v-chip>
        </div>
      </div>
      ...
      <div v-if="crew.agents && crew.agents.length > 0" class="mb-3">
        ...
        <div class="d-flex align-center flex-wrap" style="gap: 4px;">
          <v-tooltip v-for="(agent, index) in crew.agents.slice(0, 5)" :key="index" location="top">
            <template v-slot:activator="{ props }">
              <v-avatar size="32" color="primary" variant="tonal" v-bind="props" class="agent-avatar">
                <v-icon icon="mdi-robot" size="small" />
              </v-avatar>
            </template>
            {{ typeof agent === 'string' ? agent : agent.name || agent.role }}
          </v-tooltip>
          <v-chip v-if="crew.agents.length > 5" size="x-small" variant="outlined">
      ...
      <div class="d-flex align-center flex-wrap" style="gap: 8px;">
        ...
        <v-chip v-if="crew.memory?.enabled" size="x-small" variant="outlined" color="purple">
          <v-icon icon="mdi-brain" start size="small" />
          {{ $t('agent.crews.card.memory') }}
        </v-chip>
        <v-chip v-if="crew.cache?.enabled" size="x-small" variant="outlined" color="info">
    ...
    <v-card-actions class="pa-2">
      <v-btn icon size="small" variant="text" color="primary" @click="$emit('edit', crew)" :title="$t('agent.crews.card.edit')">
        <v-icon icon="mdi-pencil" />
      </v-btn>
      <v-btn icon size="small" variant="text" color="default" @click="$emit('copy', crew)" :title="$t('agent.crews.card.copy')">
        <v-icon icon="mdi-content-copy" />
      </v-btn>
      <v-btn icon size="small" variant="text" color="error" @click="$emit('delete', crew)" :title="$t('agent.crews.card.delete')">
