<template>
  <div class="deep-task-tab">
    <v-row class="mb-4" align="center">
      <v-col cols="12" md="4">
        <div class="d-flex align-center" style="gap: 8px;">
          <h2 class="text-h5 font-weight-bold">深度任务</h2>
          <v-chip size="x-small" color="purple" variant="tonal" label>Agent</v-chip>
        </div>
      ...
      <v-col cols="12" md="8" class="d-flex justify-end" style="gap: 8px;">
        ...
        <v-btn variant="outlined" @click="loadData" :loading="loading">
        ...
        <v-card class="stat-card" color="purple-lighten-5">
          <v-card-text class="d-flex align-center">
            <v-avatar color="purple" size="48" class="mr-4">
              <v-icon icon="mdi-video" color="white" />
            </v-avatar>
            <div>
      ...
      <v-table v-else hover>
        ...
        <tbody>
          <tr v-for="task in paginatedTasks" :key="task.id">
            ...
            <td>
              <v-chip :color="getTaskTypeColor(task.task_type)" size="small" variant="tonal" label>
    ...
    <v-dialog v-model="showCreateDialog" max-width="600">
      <v-card>
        ...
        <v-card-text>
          <v-text-field v-model="newTask.name" label="任务名称" :rules="[v => !!v || '请输入任务名称']" class="mb-3" />
          <v-textarea v-model="newTask.description" label="任务描述" rows="2" auto-grow class="mb-3" />
          <v-select v-model="newTask.task_type" :items="taskTypeOptions" item-title="label" item-value="value" label="任务类型" class="mb-3" />
          <v-select
          ...
          <v-select
            ...
            class="mb-3"
          />
          <v-textarea v-model="newTaskInput" label="输入数据（JSON格式）" rows="3" auto-grow placeholder='{"key": "value"}' class="mb-3" />
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn @click="showCreateDialog = false">取消</v-btn>
          <v-btn color="primary" @click="createTask" :loading="creating" :disabled="!newTask.name">创建</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
