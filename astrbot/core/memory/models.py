...
class MemoryEvent(SQLModel, table=True):
    ...
    type: str = Field(
        index=True
    )  # user_message | agent_reply | tool_call | scheduled_task | system_event
...
class MemoryScene(SQLModel, table=True):
    ...
    embedding: str | None = Field(default=None, sa_column=Column(TEXT))  # JSON array
...
class MemorySemanticClaim(SQLModel, table=True):
    ...
    source_scene_id: str | None = Field(default=None, index=True)
...
class MemoryProceduralRule(SQLModel, table=True):
    ...
    scope: str = Field(
        index=True
    )  # global_policy | shared_rule | agent_private | task_playbook
